...
static int
s_recvmsg_peer (zmq::socket_base_t *s_, const void *routing_id,
                       const int routing_id_len,
                       zmq_msg_t *msg_,
                       int flags_)
{
    int rc = s_->recv_peer (routing_id, routing_id_len, (zmq::msg_t *) msg_, flags_);
    if (unlikely (rc < 0))
        return -1;
...
int zmq_msg_recv_peer (zmq_msg_t *msg_, void *s_,
                                     ...
                                     int flags_)
{
    ...
    if (!s)
        return -1;
    return s_recvmsg_peer (s, routing_id, routing_id_len, msg_, flags_);
}


int zmq_recv_peer (void *s_, const void *routing_id, const int routing_id_len,
                       void *buf_,
                       size_t len_,
                       int flags_)
{
    ...
    int nbytes = s_recvmsg_peer (s, routing_id, routing_id_len, &msg, flags_);
    if (unlikely (nbytes < 0)) {
        int err = errno;
        rc = zmq_msg_close (&msg);
        errno_assert (rc == 0);
